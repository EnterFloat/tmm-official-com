    ...
    if (purchase === undefined) {
      ...
      getAuth0User()
        ...
        .then(result => {
          ...
          if (result == 'not_stripe_customer') {
            console.log(
              'Not a Stripe customer anymore. No previous purchases to display'
            );
            return;
          } else {
      ...
      getAuth0User()
        ...
        .then(result => {
          if (result == 'no_faunadb_user') {
            console.log(
              'Not a FaunaDB user. Creating Stripe customer and FaunaDB user'
            );
        ...
        .then(result => {
          if (result == 'not_stripe_customer') {
            console.log(
              'Not a Stripe customer anymore. Creating Stripe customer and FaunaDB user'
            );
            return createCustomer(auth0_id, auth0_email);
          } else {
            return result;
        ...
        .then(result => {
          console.log(result);
          console.log(
            'Time to create session, buy product, and reload localStorage'
          );
          resolve(result);
        })
        .catch(err => {
